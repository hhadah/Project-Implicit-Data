...
Sexuality_IAT <- fread(Sexuality_Implicit_Harvard)

# filter incomplete responses
Sexuality_IAT <- Sexuality_IAT[session_status == "C"]
Sexuality_IAT <- as.data.frame(Sexuality_IAT)

# keep some variables
Sexuality_IAT <- Sexuality_IAT %>%
  select(session_id, D_biep.Straight_Good_all, D_biep.Straight_Good_36,
         att_7, D_biep.Straight_Good_47, 
  ...
  mutate(Implicit=D_biep.Straight_Good_all, # create implicit prejudice var
...
write_csv(Sexuality_IAT, file.path(datasets,"Sexuality_IAT_Clean.csv"))
